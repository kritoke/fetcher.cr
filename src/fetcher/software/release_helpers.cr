...
module Fetcher
  module Software
    ...
    module ReleaseHelpers
      ...
      def extract_tag(release : JSON::Any) : String
        release["tag_name"]?.try(&.as_s) || ""
      end

      # Extract name, falling back to tag if name is absent
      def extract_name(release : JSON::Any) : String
      ...
      def extract_html_url(release : JSON::Any) : String
      ...
      def parse_release_date(release : JSON::Any) : Time?
      ...
      def prerelease?(release : JSON::Any) : Bool
